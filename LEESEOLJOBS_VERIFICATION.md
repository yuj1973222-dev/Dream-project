...
`LeeSeolJobs`가 survival 서버에서 로드되고, 광질/농사/낚시/탐험 기반 활동
보상 루프를 제공할 준비가 되었는지 확인한다.
...
| jar 빌드 | PASS | `target/LeeSeolJobs-0.1.0.jar`, 34K |
| 배포 경로 | PASS | `/opt/minecraft/server/plugins/LeeSeolJobs-0.1.0.jar` |
| 백업 | PASS | 2026-06-11 배포 시 기존 jar/config를 `/opt/minecraft/backups/`에 타임스탬프 백업 |
| 서비스 | PASS | `minecraft: active` |
| newworld | PASS | `inactive` 유지 |

2026-06-11 활동/탐험 패스 배포 확인:

| 항목 | 상태 | 근거 |
|---|---|---|
| 활동 별칭 | PASS | `plugin.yml`에 `/activity`, `/activities`, `/expedition`, `/explore` alias 포함 |
| 탐험 클래스 | PASS | jar에 `me/leeseol/jobs/listener/ExplorationListener.class` 포함 |
| 서버 설정 | PASS | `/opt/minecraft/server/plugins/LeeSeolJobs/config.yml`에 `exploration` 및 `[활동]` 문구 반영 |
| Quest 문구 | PASS | survival/lobby `LeeSeolQuest/config.yml`에 "일일 활동", "활동 보상" 반영 |
| 최신 survival 로그 | PASS | 21:00:28 `LeeSeolQuest enabled`, 21:00:30 `LeeSeolJobs enabled`, 21:01:04 `Done` |
| 최신 lobby 로그 | PASS | 20:59:49 `LeeSeolQuest enabled`, 20:59:54 `Done` |
...
| 탐험 보상 | IMPLEMENTED | 허용 월드에서 오늘 처음 들어간 바이옴 기준 |
...
| 덜 자란 작물 파괴 | 돈 미지급 |
| 낚시 성공 | 돈 지급 |
| 오늘 처음 들어간 바이옴 | 탐험 보상 지급, 같은 바이옴 당일 중복 미지급 |
...
2. 탐험 보상 `new-biome-daily: 600`, 일일 한도 `exploration: 15000`이
   이동 플레이에 과하거나 약한지 측정.
3. 필요 시 `/jobs top` 구현.
