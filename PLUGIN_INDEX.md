...
| Queue / lobby movement | backend names, `leeseol:queue` actions, `queue.properties`, `network.properties`; Proxy owners `NetworkRouteService`, `QueueCoordinator`, `QueuePluginMessageBridge` | `mvn -f LeeSeolProxy/pom.xml test`, `/survival`, queue/limbo enter, `/lobby`, queue leave, kicked fallback |
| Resource pack | Velocity offer URL/hash/prompt, hosted pack file; Proxy owners `ResourcePackCoordinator`, `ResourcePackOfferService`, `ResourcePackSettings` | `mvn -f LeeSeolProxy/pom.xml test`, Velocity plugin load, pack offer on join, resourcepack service response |
| Core common Paper | `/serverinfo`, `/survivalspawn`, `/lscore`/`/leeseolcore`, BungeeCord `Connect`, portal/launchpad/server-menu/server-NPC config surfaces | Core shared/spawn contract smoke, `version LeeSeolCore`, `/serverinfo`, `/survivalspawn`, `/leeseolcore servernpc list`; verify portal/launchpad/menu only if touched |
...
| --- | --- | --- | --- | --- |
| `LeeSeolCore` | survival + lobby | Common Paper core: server info/status, survival spawn/return, portal triggers, launch pads, Shift+F server menu, Citizens server NPCs, `/content`; commands `/serverinfo`, `/survivalspawn`, `/lscore`, `/leeseolcore`, `/content` | `LeeSeolCorePlugin.java`, `CoreServices.java`, `command/` admin router + portal/launchpad handlers, `status/`, `networkmove/`, `content/`, `portal/`, `launchpad/`, `menu/`, `servernpc/`, `spawn/`, `listener/`, `config/CoreConfigWriter.java`, `config.yml`, `plugin.yml`; contracts: `contents.yml`, BungeeCord `Connect`, WorldGuard/BlueMap content regions, spawn/return config keys | `mvn -f LeeSeolCore/pom.xml test package`, `version LeeSeolCore`, `/content list`, `/survivalspawn`, `/leeseolcore servernpc list`; verify portal/launchpad/menu only if touched |
| `LeeSeolTown` | survival | Party/town membership, nation colors, nation core-gated claims, treasury/upkeep/debt suspension, invasion/total war, chat channels, territory actionbar/scoreboard, neutral zones, structure core placement/undo; commands `/town`, `/party`, `/village`, `/towny`, `/tc`, `/pc`, `/nc` | `LeeSeolTownPlugin.java`, `command/`, `command/NationClaimCommand.java`, `service/` split: `TownService` facade, `TownDomainQuery`, `TownMembershipService`, `NationService`, `ClaimService`, `WarService`, `TownDisplayService`; `service/TerritoryTransition.java`, `storage/TownStore.java`, `listener/`, `hook/`, `diagnostic/`, `scoreboard/`, `structure/`, `config.yml`, `plugin.yml`; contracts: `%leeseoltown_*%`, shared town data keys `towns`/`nations`/`wars`, `beacon-claim`, `upkeep.*`, `debt.*`, `functions-suspended`, Core `contents.yml`, ItemsAdder structure ids, WorldGuard/BlueMap, Vault | `version LeeSeolTown`, `/party me`, `/party claimprice`, `/party nation claimprice`, `/party nation claim`, `/party nation deposit`, `/party nation upkeep`, `/party war declare <nation> invasion`, `/party diag`, PAPI parse, scoreboard/chat/territory/claim smoke |
...
| `LeeSeolProxy` | velocity | Velocity bootstrap facade, server list, lobby/survival movement, maintenance/fallback, queue/limbo bridge, resource-pack offer; commands `/servers`, `/lobby`, `/survival` | `LeeSeolProxyPlugin.java`, `ProxyServices.java`, `command/ProxyCommandRegistrar.java`, `network/NetworkRouteService.java`, `queue/QueueCoordinator.java`, `queue/QueuePluginMessageBridge.java`, `queue/SurvivalQueueController.java`, `resourcepack/ResourcePackCoordinator.java`, `resourcepack/ResourcePackOfferService.java`, `config/ProxyConfigRepository.java`, `velocity-plugin.json`; contracts: `leeseol:queue`, `network.properties`, `queue.properties`, `resourcepack.properties` | `mvn -f LeeSeolProxy/pom.xml test package`, descriptor smoke, `/servers`, `/survival`, `/lobby`, resource-pack offer, queue/limbo roundtrip |
